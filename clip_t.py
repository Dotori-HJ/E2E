...
def unfold(tensor: torch.Tensor, size: int, step: int, dilation: int = 1) -> torch.Tensor:
    ...
    num_strided = new_size[0] * step + (size - step)

    if numel < size:
        pad_tensor = torch.cat((tensor, torch.full((size - numel,), fill_value=-1)))
        return torch.cat((tensor, pad_tensor), dim=-1).unsqueeze(0)
    else:
        result = torch.as_strided(tensor, new_size, new_stride)
        if num_strided != numel:
            pad_tensor = tensor[num_strided - (size - step):]
            pad_tensor = torch.cat((pad_tensor, torch.full((size - pad_tensor.numel(),), fill_value=-1)))
            result = torch.cat((result, pad_tensor.unsqueeze(0)), dim=0)
        return result
...
class VideoClips:
    ...
    def get_clip(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, Any], int]:
        ...
        assert backend == "pyav", f"Only support pyav! but backend={backend}"
        ...
        num_pad = (clip_pts == -1).sum().item()

        start_pts = clip_pts[0].item()
        end_pts = clip_pts[clip_pts.size(0) - num_pad - 1].item()
        video, audio, info = read_video(video_path, start_pts, end_pts)

        if self.frame_rate is not None:
            resampling_idx = self.resampling_idxs[video_idx][clip_idx]
            num_pad = (resampling_idx == -1).sum().item()
            resampling_idx = resampling_idx[:resampling_idx.size(0) - num_pad]
        ...
        if len(video) != self.num_frames:
            pad_tensor = torch.full((self.num_frames - len(video), *video.shape[1:]), fill_value=128)
        ...
        return video, info, video_idx, clip_idx
...
# # video_pts = torch.tensor([i for i in range(129)])
# # num_frames = 128
# # step = 128 - 96
# # clips = unfold(video_pts, num_frames, step)
# # print(clips)
# # exit()
# paths = ["/home/ds/HDD2/THUMOS14/TH14_test_set_mp4/video_test_0000001.mp4"]
# clips = VideoClips(paths, 128, 96, 13)
# print(clips)
# print(len(clips))
# video, info, video_idx, clip_idx = clips.get_clip(5)
# print(video.size(), video_idx, clip_idx)

from datasets.data_utils import get_dataset_info
from datasets.raw_dataset import TADDataset

subset_mapping, feature_info, ann_file, ft_info_file = get_dataset_info("thumos14", "video")
mode = 'train'
dataset = TADDataset(
    subset_mapping[mode], mode, feature_info, ann_file, ft_info_file, fps=10, transforms=None,
    online_slice=True, slice_len=128, slice_overlap=0.75 if mode=='train' else 0.25,
    binary=False,
    input_type="video",
    resize=256,
    crop_size=224,
    rand_augment_param="rand-m7-n3-mstd0.5-inc1",
    fix_transform=False,
)
print(len(dataset))