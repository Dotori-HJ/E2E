...
def apply_nms(dets_arr, nms_thr=0.4, use_soft_nms=False, min_score=0.001):
    ...
    for cls in unique_classes:
        ...
        if not use_soft_nms:
            ...
            this_cls_dets_kept = soft_nms(this_cls_dets, 0.5, 0, 0, 100)
...
def seg_voting(nms_segs, all_segs, all_scores, iou_threshold, score_offset=1.5):
    ...
    ex_nms_segs = np.broadcast_to(nms_segs[:, np.newaxis], (num_nms_segs, num_all_segs, 2))
    # ex_all_segs = all_segs[None, :].expand(num_nms_segs, num_all_segs, 2)
    ex_all_segs = np.broadcast_to(all_segs[np.newaxis, :], (num_nms_segs, num_all_segs, 2))
    ...
    seg_weights /= np.sum(seg_weights, axis=1, keepdims=True)
