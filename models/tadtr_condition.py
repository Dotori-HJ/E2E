...
from models.matcher_condition import build_matcher
...
from util.segment_ops import segment_clr_to_t1t2, segment_cw_to_t1t2, segment_iou

from .custom_loss import sigmoid_focal_loss
from .tdm import FPN, TDM
from .transformer_condition import build_deformable_transformer
...
class TadTR(nn.Module):
    ...
    def __init__(self, backbone, position_embedding, transformer, num_classes, num_queries, feature_dim, num_patterns=0,
                 aux_loss=True, with_segment_refine=True, with_act_reg=False,
                 two_stage=False, use_dab=True, mixed_selection=False):
        ...
        # self.class_embed = MLP(hidden_dim, hidden_dim, num_classes, num_layers=3)
        self.segment_embed = MLP(hidden_dim, hidden_dim, 3, num_layers=3)
        ...
        #     lateral_norm_layer=nn.BatchNorm1d,
        #     # fpn_norm_layer=,
        # )
        ...
        self.input_proj = nn.ModuleList([
            nn.Sequential(
                nn.Conv1d(backbone.num_channels, hidden_dim, kernel_size=3, padding=1),
        ...
        if isinstance(self.class_embed, MLP):
        ...
        if two_stage:
            ...
            for segment_embed in self.segment_embed:
                nn.init.constant_(segment_embed.layers[-1].bias.data[1:], 0.0)
            nn.init.constant_(self.segment_embed[-1].layers[-1].bias.data[1:], -2.0)
        ...
        # self.tuner = MixerTuner(
        #     feature_dims=[2048], middle_dim=512, temporal_length=128, num_layers=2, num_levels=1
        # )
    ...
    def _to_roi_align_format(self, rois, T, scale_factor=1):
        ...
        rois_left = rois[:, :, 1:2] * scale_factor
        rois_right = rois[:, :, 2:3] * scale_factor
        rois_abs = torch.cat(
            (rois_center - rois_left, rois_center + rois_right), dim=2) * T
        # expand the RoIs
    ...
    def forward(self, samples: NestedTensor):
        ...
        # masks = [mask]
        # pos = [self.position_embedding(NestedTensor(src, mask)) for src, mask in zip(srcs, masks)]
        ...
        if self.mixed_selection and self.two_stage:
            query_embeds = self.query_embed.weight
        elif self.two_stage:
            query_embeds = None
        elif self.use_dab:
            if self.num_patterns == 0:
                query_embeds = tgt_embed = self.tgt_embed.weight           # nq, 256
                ...
                refanchor = self.refpoint_embed.weight.repeat(self.num_patterns, 1)      # nq*num_pat, 1
...
class SetCriterion(nn.Module):
    ...
    def loss_segments(self, outputs, targets, indices, num_segments):
        ...
        loss_segment = F.l1_loss(
            segment_clr_to_t1t2(src_segments),
            segment_cw_to_t1t2(target_segments),
            reduction='none'
        )
    ...
    def loss_actionness(self, outputs, targets, indices, num_segments):
        ...
        src_segments = outputs['pred_segments'].reshape((-1, 3))
    ...
    def forward(self, outputs, targets):
        ...
        for target in targets:
            target["segments"] = target["segments"].repeat(4, 1)
            target["labels"] = target["labels"].repeat(4)
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, fuse_score=True):
        ...
        segments = segment_clr_to_t1t2(out_segments)   # bs, nq, 2
        ...
        if cfg.postproc_rank == 1:     # default
            ...
            topk_values, topk_indexes = torch.topk(prob.view(
                ...
                out_logits.shape[0], -1), prob.shape[1]*prob.shape[2], dim=1)
            scores = topk_values
            ...
            query_ids = torch.div(torch.arange(0, cfg.postproc_cls_topk*segments.shape[1], 1, dtype=labels.dtype,
                         device=labels.device), cfg.postproc_cls_topk, rounding_mode='trunc')[None, :].repeat(labels.shape[0], 1)
