...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 mixed_selection=False, use_dab=False, high_dim_query_update=False,
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, d_model=256, return_intermediate=False, look_forward_twice=False, use_dab=False, high_dim_query_update=False, no_sine_embed=False):
        super().__init__()
        ...
        if use_dab:
            self.query_scale = MLP(d_model, d_model, d_model, 1)
            if self.no_sine_embed:
                self.ref_point_head = MLP(4, d_model, d_model, 2)
            else:
                self.ref_point_head = MLP(2 * d_model, d_model, d_model, 1)
        self.high_dim_query_update = high_dim_query_update
        if high_dim_query_update:
            self.high_dim_query_proj = MLP(d_model, d_model, d_model, 1)
...
def gen_sineembed_for_position(pos_tensor):
    ...
    dim_t = 10000 ** (2 * torch.div(dim_t, 2, rounding_mode='trunc') / 256)
