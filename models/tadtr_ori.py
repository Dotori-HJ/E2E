...
from .transformer_ori import build_deformable_transformer
...
class TadTR(nn.Module):
    ...
    def forward(self, samples):
        ...
        if samples.tensors.size(1) != 384:
            with open('t.txt', 'at') as f:
                f.write(f"{samples.tensors.size()}")
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, fuse_score=True):
        ...
        # segments = torch.clip(segments, 0, vid_length.item())
