...
class Attention(nn.Module):
    def __init__(
            self, dim, num_heads=8, qkv_bias=True, qk_scale=None, attn_drop=0.,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_channels = self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
    ...
    def load_pretrained_weight(self, ckpt_path=None):
        ...
        state_dict = {k[len('encoder.'):]:v for k, v in state_dict.items() if 'decoder' not in k}

        # print(state_dict.keys() | self.state_dict().keys())
        # print(state_dict.keys() & self.state_dict().keys())
        # exit()
    ...
    def forward(self, x):
        B, _, num_frames, _, _ = x.shape
        assert num_frames % self.num_frames == 0
        num_snippets = num_frames // self.num_frames
        x = rearrange(x, "b c (t n) h w -> (b n) c t h w", t=self.num_frames, n=num_snippets)
        x = self.forward_features(x)
        x = rearrange(x, "(b n) d -> b d n", b=B, n=num_snippets)
