...
class ResNet3dSlowFast(nn.Module):
    ...
    def __init__(self,
        ...
        if pooler == 'avg':
            ...
            self.slow_poolers = nn.ModuleList([SpatialPooler('twpool', input_dim=2048, base_dim=256, num_layers=4, output_dim=2048)])
            self.fast_poolers = nn.ModuleList([SpatialPooler('twpool', input_dim=256, base_dim=32, num_layers=4, output_dim=256)])
            self.num_channels = 2304
...
if __name__ == '__main__':
    ...
    x = torch.rand([1, 3, 256, 96, 96], device=device)
